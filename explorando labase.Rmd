...
# Crear el mapa
mapa <- leaflet() %>%
  addTiles() %>% addPolygons(data = barrio_sin_z,color = "red",opacity = 1,
  dashArray = "3",   weight = 6) 
...
ggplot()+geom_sf(data = ventana,color="black",size=2)+geom_sf(data = y,color="black")+geom_sf(data = puntos_grafica,aes(),size=0.1,color="yellow")+facet_wrap(~crime_type,nrow=1,ncol=4)+
...
horadeldiabarrio<-horadeldiabarrio |> select(-geometry)
...
palette_ubicacion1 <- c("yellow", "#66CCFF")
palette_ubicacion2 <- c("#CC3333", "#FF6666")

# Crear el gráfico
horas <- ggplot(barras_horario, aes(x = crime_hour, y = Porcentaje, fill = UBICACIÓN)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Porcentaje), vjust = -0.05, color = "black", size = 4, angle = 90, position = position_dodge(width = 1)) +
  scale_fill_manual(values = c(palette_ubicacion1, palette_ubicacion2)) + 
  labs(x = "Hora del día", y = "Porcentaje") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 15, face = "bold"),  # Aumentar tamaño y poner en negrita
    axis.text.y = element_text(size = 15, face = "bold"),  # Aumentar tamaño y poner en negrita
    axis.title = element_text(size = 20, face = "bold"),   # Aumentar tamaño y poner en negrita
