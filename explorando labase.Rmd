...
horas <- ggplot(barras_horario, aes(x = crime_hour, y = Porcentaje, fill = UBICACIÓN)) +
  ...
  geom_text(aes(label = Porcentaje),hjust=-0.4 ,vjust = -0.05, color = "black", size = 4, angle = 90, position = position_dodge(width = 1)) +
  scale_fill_manual(values = c(palette_ubicacion1, palette_ubicacion2)) + 
  labs(x = "Hora del día", y = "Porcentaje %") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1, size = 15, face = "bold"),  # Aumentar tamaño y poner en negrita
